...
# sta_mur <- import(file = "data/sta_mur.csv") # dropped by Theissen
...
merge_sta1 <- full_join(sta_oel, sta_int, by = "date")
merge_sta1 <- merge_sta1 %>%
  rename(int = prcp.x) %>%
  rename(oel = prcp.y) 

merge_sta2 <- full_join(merge_sta1, sta_cot, by = "date")
merge_sta2 <- merge_sta2 %>%
  rename(cot = prcp)

merge_sta3 <- full_join(merge_sta2, sta_mis, by = "date")
merge_sta3 <- merge_sta3 %>%
  rename(mis = prcp)

merge_sta4 <- full_join(merge_sta3, sta_ora, by = "date")
merge_sta4 <- merge_sta4 %>%
  rename(ora = prcp)

sta <- full_join(merge_sta4, sta_rap, by = "date")
sta <- sta %>%
...
# merge_staA <- full_join(merge_sta9, sta_kad, by = "date")
#merge_staA <- merge_staA %>%
#  rename(kad = prcp)

#merge_sta2 <- full_join(merge_oel, sta_int, by = "date")
#merge_sta2 <- merge_sta2 %>%
#  rename(int = prcp)

#merge_sta4 <- full_join(merge_sta3, sta_lon, by = "date")
#merge_sta4 <- merge_sta4 %>%
#  rename(lon = prcp)

#merge_sta5 <- full_join(merge_sta4, sta_har, by = "date")
#merge_sta5 <- merge_sta5 %>%
#  rename(har = prcp)

#merge_sta6 <- full_join(merge_sta5, sta_ain, by = "date")
#merge_sta6 <- merge_sta6 %>%
#  rename(ain = prcp)

#merge_sta7 <- full_join(merge_sta6, sta_mur, by = "date")
#merge_sta7 <- merge_sta7 %>%
#  rename(mur = prcp)
```

```{r munge-precip-data,}
# Purpose: fill NA prior to upscaling to monthly data

# note on naming convention: 
# dummy        swap variable for testing before updating master 
# sta_xx       refers to start year (xx) to end of record
# sta_xx_yy    refers to start year (xx) to end year (yy)
# sta_xxxx     refers to data for only that year
# sta_xxxxmyy  refers that year w/ later month (yy) starting date
# sta_xxx_filt filter variable that disappears
# sta_raw      the "mostly" raw dataset
# sta_ intro   quick check of NA that disappears
# sta_sw       Oelrichs & Oral

# load metadata & data
sta_meta  <- as.tibble(import("data/sta_meta_fin.csv"))
sta_raw   <- import("data/stations.csv")
...
# sta_intro <- as.tibble(introduce(sta))
# sta_intro <- sta_intro %>%
#  select(-(1:4)) 

#  all_missing_columns total_missing_values total_observations
#               <int>                <int>              <int>
#                   0               102763             410328
...
# Test for cleaning 71 & 72
  test_ge72     <- sta_raw %>% filter(year == 1972)
  test_71m05 <- sta_raw %>% filter(year == 1971 & month >= 5)
test_ge71m05 <- bind_rows(test_ge72, test_71m05)
  rm(test_ge72, test_71m05)

test_le71    <- sta_raw %>% filter(year == 1971 & month < 5)
test         <- bind_rows(test_ge71m05, test_le71)
  rm(sta_72_bef, sta_1971m05_bef)

# Sequential cleaning starting with ORAL start date
...
sta_ge71m05     <- bind_rows(sta_ge72, sta_71m05)
  rm(sta_72, sta_1971m05)

  sta_le71      <- sta_raw %>% filter(year < 1971)
  sta_71m01_m05 <- sta_raw %>% filter(year == 1971 & month < 5)
sta_le71m05     <- bind_rows(sta_le71, sta_71m01_m05)
  rm(sta_le71, sta_71m01_m05)

#   2. Clean Oral NA - has 368 NA <- filling with Oelrichs
#        Result: 10 NA values 
sta_notNA <- sta_71 %>%
  filter(!is.na(ora))
...
sta_NA <- sta_71 %>%
  filter(is.na(ora)) %>%
  mutate(ora = oel) 
dummy <- bind_rows(sta_notNA, sta_NA)
  
sta_check <- dummy %>%
  filter(is.na(ora))
  
sta_71 <- dummy      # update with fixed Oral data

#   3. Clean Oelrichs NA - has 469 NA <- filling with Oelrichs
#        Result: 10 NA values 
sta_notNA <- sta_71 %>%
  filter(!is.na(oel))
...
#   3. Clean Oral & Oelrich remain NA - 10 NA <- filling with Harrison #        Result: Zero NA values- Oral & Oelrichs
dummy <- left_join(dummy, sta_har, by = "date")
  rm(sta_har)
sta_notNA <- dummy %>%
  filter(!is.na(ora))
...
sta_check <- dummy %>%
  filter(is.na(oel))   # checks out - zero NA for Oelrichs

sta_71 <- dummy      # update with fixed Oral & Oelrichs data

#   4. Add results together, drop Oral, and split on next oldest sta
sta_work <- rbind(sta_71, sta_71_bef)
```
...
merge_airlines <- merge(flights, airlines, by = "carrier", 
                        all.x = TRUE)
merge_planes <- merge(merge_airlines, planes, by = "tailnum",
                      all.x = TRUE, 
                      suffixes = c("_flights", "_planes"))
merge_airports_origin <- merge(merge_planes, airports, 
