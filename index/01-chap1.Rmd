...
sta_ge48m05     <- bind_rows(sta_ge49, sta_48m05) # this is active
  ...
  sta_lt49      <- sta_trans %>% filter(year < 1948)
  sta_48m01_m05 <- sta_trans %>% filter(year == 1948 & month < 5)
sta_lt48m05     <- bind_rows(sta_lt49, sta_48m01_m05) # this is not
  rm(sta_lt49, sta_48m01_m05)

#   2. filter NA vals from Rapid City with Interior
#     Before: 6 NA <- filling with Interior
#     After: xx NA values 
...
#  select(-prcp, -id) 
sta_trans4 <- bind_rows(sta_ge48m05, sta_lt48m05)
...
introduce(sta_trans4)
# rows columns total_missing_values total_observations
# 45592       9            99599            410328
