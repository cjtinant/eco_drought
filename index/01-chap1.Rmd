...
Download station data
  DL metadata completed
  DL station precip obs completed
  Voroni diagrams completed - final selection of sites

Next steps:
  Data cleaning - use Data Exploror 
  merge precip d.fs

Identify Journal  
Calculate SPI & SPE           
Learn purrr::map()            
Develop SDI function          
Watershed delineation        
RF clustering of ungaged sta.
-->

<!-- next steps
update the maps to find the data file that moved under ~index
push github
fix thesis build
-->
...
library("rnoaa") #  R wrapper for NOAA data including 
...
sta_hot <- import(file = "data/sta_hot.csv")
sta_oel <- import(file = "data/sta_oel.csv")
sta_int <- import(file = "data/sta_int.csv")
sta_cot <- import(file = "data/sta_cot.csv")
sta_lon <- import(file = "data/sta_lon.csv")
sta_har <- import(file = "data/sta_har.csv")
sta_ain <- import(file = "data/sta_ain.csv")
sta_mur <- import(file = "data/sta_mur.csv")
sta_mis <- import(file = "data/sta_mis.csv")
sta_ora <- import(file = "data/sta_ora.csv")
sta_kad <- import(file = "data/sta_kad.csv")
sta_rap <- import(file = "data/sta_rap.csv") # kind of messed up!
# sta_her <- import(file = "data/sta_her.csv") n = 304 obs so drop

# join tables and rename prcp cols - data is in mm
merge_sta1 <- full_join(sta_hot, sta_oel, by = "date")
merge_sta1 <- merge_sta1 %>%
  rename(hot = prcp.x) %>%
  rename(orl = prcp.y) 

merge_sta2 <- full_join(merge_sta1, sta_int, by = "date")
merge_sta2 <- merge_sta2 %>%
  rename(int = prcp)

merge_sta3 <- full_join(merge_sta2, sta_cot, by = "date")
...
merge_sta8 <- full_join(merge_sta7, sta_mis, by = "date")
merge_sta8 <- merge_sta8 %>%
  rename(mis = prcp)

merge_sta9 <- full_join(merge_sta8, sta_ora, by = "date")
merge_sta9 <- merge_sta9 %>%
  rename(ora = prcp)
...
sta <- full_join(merge_staA, sta_rap, by = "date")
sta <- sta %>%
...
# gather stations
sta_gath <- sta %>%
  gather(key = "key", value = "prcp", -date, na.rm = FALSE)
...
# want to look at data without long valley and kadoka
sta_filt <- sta %>%
  select(-lon) %>%
  select(-kad)

sta_ne <- sta %>%
  select(date, lon, kad, year, month)
ggplot(sta_ne, aes(kad, lon)) +
  geom_point() +
