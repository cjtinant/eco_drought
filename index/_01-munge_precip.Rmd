...
rm(sta_ge07m12, sta_lt07m12, sta_NA)
  ...
  sta_ge10      <- sta_trans6 %>% filter(year >= 1910) # yr above
  sta_09m06     <- sta_trans6 %>% filter(year == 1909 & month >= 6)
sta_fin     <- bind_rows( sta_ge10, sta_09m06) # this is active
  rm(sta_ge10, sta_09m06)
  
#   5. Clean up final precip dataset
sta_fin <- sta_fin %>%
  select(-mur) %>%
  arrange(date)
...
#   rows columns total_missing_values total_observations
#  <int>   <int>                <int>              <int>
# 39812       9                66992             358308
export(sta_fin, file = "data/stations_final.csv") 
```

```{r daily2monthly-precip}
# General Purpose: convert daily precip to monthly precip
# Specific purpose 

# load metadata & data
sta_meta  <- as.tibble(import("data/sta_meta_fin.csv"))
sta_day       <- as.tibble(import("data/stations_final.csv"))
...
# gather daily values
sta_gath <- gather(sta_day, key = "station", value = "prcp", -date, -year, -month, factor_key = TRUE)
