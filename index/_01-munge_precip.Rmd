...
```{r eval-lon-cont, eval = FALSE}
# General Purpose: fill NA prior to upscaling to monthly data
# Specific purpose - Check Hot Springs - Annual precip below for OEL
# reflects Black Hills rather than rainshadow
# Variable naming convention - see munge-precip-data-oral code chunk  
...
sta_52 <- sta_gath_52 %>%
  spread(station, prcp) %>%
  select(rap, oel, int, cot, mis)

# create a correlation matrix and plot it
sta_M <- cor(sta_52)
corrplot(sta_M, method = "number", type = "upper")

# Results: Mission is a lot different from other stations 
